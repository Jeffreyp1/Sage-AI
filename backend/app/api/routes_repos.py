...
from app.config import get_local_scan_workspace_root, get_settings
...
def scan_local(request: ScanLocalRequest, db: Session = Depends(get_db)) -> ScanLocalResponse:
    ...
    try:
        workspace_root = get_local_scan_workspace_root(settings)
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    try:
        result = ScanService().scan_local(
            request.path,
...
def scan_github(request: ScanGitHubRequest, db: Session = Depends(get_db)) -> ScanLocalResponse:
    ...
    with tempfile.TemporaryDirectory(prefix="vulnsage-github-") as temp_dir:
        ...
        try:
            result = ScanService().scan_local(str(repo_path), workspace_root=temp_dir)
    ...
    return [
        {
            ...
            "parent_package": public_identifier_text(package.parent_package),
        }
