...
from app.services.dependency_parser import ParsedDependency, infer_version_from_spec, parse_dependencies
...
class ScanService:
    ...
    def scan_local(self, repo_path: str) -> ScanResult:
        ...
        for dependency in packages:
            query_version = dependency.current_version or (
                infer_version_from_spec(dependency.version_spec or "")
            )
            try:
        ...
        summary = build_summary(packages=packages, vulnerabilities=deduped_all, tasks=tasks)
...
def build_summary(
    ...
    return {
        ...
        "raw_alerts": len(vulnerabilities),
